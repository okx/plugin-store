#!/usr/bin/env bash
# OKOne MR build job — Node plugins (compiled via Bun, identical recipe to TS).
# Mirrors plugin-store/.github/workflows/plugin-build.yml :: build-node
...
[ "${LANG}" = "node" ] || { echo "lang=${LANG} (not node), skipping"; exit 0; }
...
echo "OK: node build → ${OUT}/${BINARY_NAME}"
