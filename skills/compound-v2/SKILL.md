...
if ! command -v compound-v2 >/dev/null 2>&1; then
  ...
  mkdir -p ~/.local/bin
  curl -fsSL "https://github.com/MigOKG/plugin-store/releases/download/plugins/compound-v2@0.1.0/compound-v2-${TARGET}${EXT}" -o ~/.local/bin/compound-v2${EXT}
  chmod +x ~/.local/bin/compound-v2${EXT}
fi
```
