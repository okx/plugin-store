...
pub async fn run(chain_id: u64) -> anyhow::Result<()> {
    ...
    let price_result = onchainos::eth_call(chain_id, config::ORACLE_ADDRESS, &price_calldata).await?;
    ...
    match rpc::extract_return_data(&price_result) {
        Ok(hex) => match rpc::decode_uint256(&hex) {
            Ok(price_wei) => {
                ...
                if let Ok(mint_result) = onchainos::eth_call(chain_id, config::DEPOSIT_POOL_ADDRESS, &mint_calldata).await {
