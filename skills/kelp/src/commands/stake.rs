...
pub async fn run(args: StakeArgs) -> anyhow::Result<()> {
    ...
    // Convert ETH to wei
    if args.amount.trim().is_empty() || args.amount.trim() == "0" {
        anyhow::bail!("Stake amount must be greater than 0");
    }
    let amount_wei = config::parse_units(&args.amount, 18)?;
    let amount_f64: f64 = args.amount.trim().parse().unwrap_or(0.0);
    ...
    let expected_rseth_wei = match onchainos::eth_call(chain_id, config::DEPOSIT_POOL_ADDRESS, &mint_calldata).await {
    ...
    let price_eth = match onchainos::eth_call(chain_id, config::ORACLE_ADDRESS, &price_calldata).await {
        Ok(result) => match rpc::extract_return_data(&result) {
    ...
    println!("Expected rsETH:    {:.8} rsETH", if expected_rseth > 0.0 { expected_rseth } else { amount_f64 / price_eth });
    ...
    println!("You will receive approximately {:.8} rsETH.", if expected_rseth > 0.0 { expected_rseth } else { amount_f64 / price_eth });
    println!();
