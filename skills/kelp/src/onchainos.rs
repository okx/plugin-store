...
/// Resolve the wallet address for a given chain_id using `onchainos wallet addresses`.
/// If dry_run is true, returns the zero address immediately without calling onchainos.
...
/// Send a write transaction via `onchainos wallet contract-call`.
/// dry_run=true: returns a simulated success response immediately (no broadcast).
/// amt: optional ETH value in wei (for payable functions).
