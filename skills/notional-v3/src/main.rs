...
struct Cli {
    ...
    /// Confirm and broadcast write transactions (without this flag, shows preview only)
    #[arg(long)]
    confirm: bool,
...
async fn main() -> anyhow::Result<()> {
    ...
    match &cli.command {
        ...
        Commands::EnterPosition(args) => {
            commands::enter_position::execute(args, cli.dry_run, cli.confirm, cli.chain).await?;
        }
        Commands::ExitPosition(args) => {
            commands::exit_position::execute(args, cli.dry_run, cli.confirm, cli.chain).await?;
        }
        Commands::InitiateWithdraw(args) => {
            commands::initiate_withdraw::execute(args, cli.dry_run, cli.confirm, cli.chain).await?;
        }
        Commands::ClaimRewards(args) => {
            commands::claim_rewards::execute(args, cli.dry_run, cli.confirm, cli.chain).await?;
        }
    }
