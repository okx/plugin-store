...
pub async fn execute(args: &ExitPositionArgs, dry_run: bool, confirm: bool, chain_id: u64) -> anyhow::Result<()> {
    if dry_run {
    ...
    if !confirm && !dry_run {
        println!("{}", serde_json::to_string_pretty(&json!({
            ...
            "action": "exit-position", "vault": args.vault, "shares": shares_raw.to_string()
        }))?);
        return Ok(());
    }
