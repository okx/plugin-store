...
pub async fn execute(args: &EnterPositionArgs, dry_run: bool, confirm: bool, chain_id: u64) -> anyhow::Result<()> {
    ...
    let amount_raw = config::parse_units(&args.amount, decimals)?;
    let borrow_raw = config::parse_units(&args.borrow_amount, decimals)?;

    if !confirm && !dry_run {
        println!("{}", serde_json::to_string_pretty(&json!({
            "ok": true, "preview": true,
            "message": "Add --confirm to broadcast",
            "action": "enter-position", "vault": args.vault, "amount": args.amount
        }))?);
        return Ok(());
    }
