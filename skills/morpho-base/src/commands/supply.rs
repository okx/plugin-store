...
pub async fn run(
    ...
    if !confirm && !dry_run {
        println!("{}", serde_json::json!({
            ...
            "operation": "supply", "vault": vault, "amount": amount
        }));
        return Ok(());
    }
    ...
    let approve_result = onchainos::wallet_contract_call(chain_id, &asset_addr, &approve_calldata, from, None, dry_run, confirm).await?;
    ...
    let deposit_result = onchainos::wallet_contract_call(chain_id, vault, &deposit_calldata, from, None, dry_run, confirm).await?;
