...
pub async fn run(
    ...
    if !confirm && !dry_run {
        println!("{}", serde_json::json!({
            ...
            "operation": "repay", "marketId": market_id, "amount": display_amount
        }));
        return Ok(());
    }
    ...
    let approve_result = onchainos::wallet_contract_call(chain_id, &loan_token, &approve_calldata, from, None, dry_run, confirm).await?;
