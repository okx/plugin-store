...
pub async fn run(
    ...
    if !confirm && !dry_run {
        println!("{}", serde_json::json!({
            ...
            "operation": "withdraw", "vault": vault, "amount": display_amount
        }));
        return Ok(());
    }

    let result = onchainos::wallet_contract_call(chain_id, vault, &calldata_hex, from, None, dry_run, confirm).await?;
