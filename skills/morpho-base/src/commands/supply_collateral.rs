...
pub async fn run(
    ...
    if !confirm && !dry_run {
        println!("{}", serde_json::json!({
            ...
            "operation": "supply-collateral", "marketId": market_id, "amount": amount
        }));
        return Ok(());
    }
    ...
    let approve_result = onchainos::wallet_contract_call(chain_id, &collateral_token, &approve_calldata, from, None, dry_run, confirm).await?;
