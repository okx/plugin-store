...
pub async fn resolve_wallet(from: Option<&str>, chain_id: u64) -> anyhow::Result<String> {
    ...
    // Use `wallet addresses` to reliably get the EVM address
    let output = tokio::process::Command::new("onchainos")
    ...
    let v: Value = serde_json::from_str(&stdout)
        .map_err(|e| anyhow::anyhow!("Failed to parse onchainos wallet addresses: {}", e))?;
    // EVM addresses are under data.evm[]
    if let Some(arr) = v["data"]["evm"].as_array() {
        // Find the address matching our chain_id, or fall back to first EVM address
        for item in arr {
            let idx = item["chainIndex"].as_str()
                .and_then(|s| s.parse::<u64>().ok())
                .or_else(|| item["chainIndex"].as_u64());
            if idx == Some(chain_id) {
    ...
    // Secondary fallback: wallet balance --chain
    let chain_str = chain_id.to_string();
    let output2 = tokio::process::Command::new("onchainos")
        .args(["wallet", "balance", "--chain", &chain_str])
        .output()
        .await?;
    let stdout2 = String::from_utf8_lossy(&output2.stdout);
    let v2: Value = serde_json::from_str(&stdout2)?;
    if let Some(addr) = v2["data"]["address"].as_str() {
        if !addr.is_empty() {
            return Ok(addr.to_string());
        }
    }
