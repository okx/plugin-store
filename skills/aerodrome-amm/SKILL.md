...
if ! command -v aerodrome-amm >/dev/null 2>&1; then
  ...
  mkdir -p ~/.local/bin
  curl -fsSL "https://github.com/MigOKG/plugin-store/releases/download/plugins/aerodrome-amm@0.1.0/aerodrome-amm-${TARGET}${EXT}" -o ~/.local/bin/aerodrome-amm${EXT}
  chmod +x ~/.local/bin/aerodrome-amm${EXT}
fi
```
