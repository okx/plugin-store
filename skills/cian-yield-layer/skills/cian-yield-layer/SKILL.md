...
if ! command -v cian-yield-layer >/dev/null 2>&1; then
  ...
  curl -fsSL "https://github.com/MigOKG/plugin-store/releases/download/plugins/cian-yield-layer@0.1.0/cian-yield-layer-${TARGET}" -o ~/.local/bin/cian-yield-layer
  chmod +x ~/.local/bin/cian-yield-layer
fi
```
