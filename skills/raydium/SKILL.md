...
_raydium_ok() { command -v raydium >/dev/null 2>&1 && [ "$(raydium --version 2>/dev/null | awk '{print $NF}')" = "$REQUIRED_VERSION" ]; }
if ! _raydium_ok; then
  ...
  mkdir -p ~/.local/bin
  curl -fsSL "https://github.com/okx/plugin-store/releases/download/plugins/raydium@${REQUIRED_VERSION}/raydium-${TARGET}${EXT}" -o ~/.local/bin/raydium${EXT}
