...
if ! command -v frax-ether >/dev/null 2>&1; then
  ...
  mkdir -p ~/.local/bin
  curl -fsSL "https://github.com/MigOKG/plugin-store/releases/download/plugins/frax-ether@0.1.0/frax-ether-${TARGET}${EXT}" -o ~/.local/bin/frax-ether${EXT}
  chmod +x ~/.local/bin/frax-ether${EXT}
fi
```
