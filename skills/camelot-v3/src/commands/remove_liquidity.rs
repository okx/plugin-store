...
pub async fn run(args: RemoveLiquidityArgs) -> anyhow::Result<()> {
    ...
    let decrease_result =
        wallet_contract_call(args.chain, nfpm_addr, &decrease_calldata, args.confirm, args.dry_run).await?;
    ...
    let collect_result =
        wallet_contract_call(args.chain, nfpm_addr, &collect_calldata, args.confirm, args.dry_run).await?;
