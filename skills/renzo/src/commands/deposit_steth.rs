...
pub async fn run(args: DepositStethArgs) -> anyhow::Result<()> {
    ...
    let amount_wei = (args.amount * 1e18).round() as u128;

    // Pre-flight: check paused()
    let paused_calldata = format!("0x{}", config::SEL_PAUSED);
    if let Ok(result) = onchainos::eth_call(config::RESTAKE_MANAGER, &paused_calldata, config::RPC_URL).await {
    ...
    let allowance_result =
        onchainos::eth_call(config::STETH_ADDRESS, &allowance_calldata, config::RPC_URL).await?;
