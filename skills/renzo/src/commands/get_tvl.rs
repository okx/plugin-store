...
pub async fn run() -> anyhow::Result<()> {
    ...
    let result = onchainos::eth_call(config::RESTAKE_MANAGER, &calldata, config::RPC_URL).await?;
    ...
    let supply_result = onchainos::eth_call(config::EZETH_ADDRESS, &supply_calldata, config::RPC_URL).await?;
