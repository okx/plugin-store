...
pub async fn run(args: BalanceArgs) -> anyhow::Result<()> {
    ...
    let ezeth_result = onchainos::eth_call(config::EZETH_ADDRESS, &ezeth_calldata, config::RPC_URL).await?;
    ...
    let steth_result = onchainos::eth_call(config::STETH_ADDRESS, &steth_calldata, config::RPC_URL).await?;
