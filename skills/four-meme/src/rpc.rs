...
pub fn decode_uint256(hex_data: &str, slot: usize) -> u128 {
    ...
    // Parse only the lower 32 hex chars to avoid u128 overflow on large values
    let lower = &chunk[32..];
    u128::from_str_radix(lower, 16).unwrap_or(0)
}

/// Decode a full u256 as a string (for display) from slot index
pub fn decode_uint256_str(hex_data: &str, slot: usize) -> String {
    let data = hex_data.trim_start_matches("0x");
    let start = slot * 64;
    if data.len() < start + 64 {
        return "0".to_string();
    ...
    let chunk = &data[start..start + 64];
    // Parse last 16 bytes (fits u128 for most values)
    let lower = &chunk[32..];
    u128::from_str_radix(lower, 16)
        .map(|v| v.to_string())
        .unwrap_or_else(|_| format!("0x{}", chunk))
}
