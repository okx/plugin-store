...
/// Selector: keccak256("buyTokenAMAP(address,uint256,uint256)") = 0x87f27655
...
/// Selector: keccak256("sellToken(address,uint256,uint256)") = 0x3e11741f
...
/// Selector: keccak256("tryBuy(address,uint256,uint256)") = 0xe21b103a
...
/// Selector: keccak256("trySell(address,uint256)") = 0xc6f43e8c
...
/// Selector: keccak256("getTokenInfo(address)") = 0x1f69565f
pub fn build_get_token_info_calldata(token: &str) -> String {
    format!("0x1f69565f{}", pad_address(token))
}

/// Parse a BNB string like "0.001" into wei (u128), no float used.
/// Truncates to gwei precision (9 decimal places) to avoid "GW - GWEI" alignment errors.
...
pub fn format_wei_as_bnb(wei: u128) -> String {
    ...
    if frac == 0 {
        ...
        let display_len = trimmed.len().min(9);
        format!("{}.{}", whole, &trimmed[..display_len])
    }
}
