...
pub async fn run(args: &BuyArgs) -> anyhow::Result<()> {
    ...
    // Use amountMsgValue from tryBuy if available, else use funds_wei
    ...
    let cd_len = calldata.len();
    println!("Calldata:     {}...{}", &calldata[..10.min(cd_len)], &calldata[cd_len.saturating_sub(8)..]);
