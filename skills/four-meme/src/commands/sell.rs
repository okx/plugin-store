...
use crate::config::{CHAIN_ID, BSC_RPC, TOKEN_MANAGER_HELPER_V3, TOKEN_MANAGER_V2, ZERO_ADDRESS};
use crate::calldata::{
...
pub struct SellArgs {
    ...
    /// Amount of tokens to sell (in whole token units, e.g. "100000" for 100k tokens)
    #[arg(long)]
    pub amount: String,
...
/// Parse token amount string (whole tokens) to raw units (18 decimals)
...
pub async fn run(args: &SellArgs) -> anyhow::Result<()> {
    ...
    let tm_to_use = if token_manager != ZERO_ADDRESS {
        token_manager.clone()
    } else {
        TOKEN_MANAGER_V2.to_string()
    };
    ...
    println!("\n--- Transaction Plan ---");
    println!("Step 1: Approve TokenManager to spend tokens");
    println!("Step 2: Call sellToken on TokenManager");
    let sc_len = sell_calldata.len();
    println!("Sell Calldata: {}...{}", &sell_calldata[..10.min(sc_len)], &sell_calldata[sc_len.saturating_sub(8)..]);
    ...
    // Only proceed to sell if approve was confirmed/dry-run
    if args.confirm || args.dry_run {
        if let Some(tx_hash) = approve_result["data"]["txHash"].as_str() {
            ...
            if tx_hash != zero_tx && !tx_hash.is_empty() {
