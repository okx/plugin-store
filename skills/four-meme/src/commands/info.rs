...
use crate::config::{API_TOKEN_GET, BSC_RPC, TOKEN_MANAGER_HELPER_V3};
...
pub async fn run(args: &InfoArgs) -> anyhow::Result<()> {
    ...
    if api_code == 0 {
        ...
        if let Some(price_info) = d["tokenPrice"].as_object() {
            ...
            if let Some(progress) = price_info.get("progress").and_then(|v| v.as_str()) {
                let pct = if let Ok(p) = progress.parse::<f64>() {
                    format!("{:.1}%", p * 100.0)
                } else {
                    progress.to_string()
                };
                println!("BC Progress:  {}", pct);
            }
    ...
    if onchain_raw.len() > 2 {
        ...
        if version > 0 {
            ...
            let quote_display = if quote == crate::config::ZERO_ADDRESS {
            ...
            let fee_pct = fee_rate as f64 / 10000.0;
            println!("Fee Rate:     {:.2}%", fee_pct);

            if max_offers > 0 {
                let sold = max_offers.saturating_sub(offers);
                let pct = sold as f64 / max_offers as f64 * 100.0;
                println!("Tokens Sold: {} / {} ({:.1}%)",
                    format_token_amount(sold * 1_000_000_000_000_000_000u128),
                    format_token_amount(max_offers * 1_000_000_000_000_000_000u128),
                    pct
                );
            }
