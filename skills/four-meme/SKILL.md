...
if ! command -v four-meme >/dev/null 2>&1; then
  ...
  mkdir -p ~/.local/bin
  curl -fsSL "https://github.com/MigOKG/plugin-store/releases/download/plugins/four-meme@0.1.0/four-meme-${TARGET}${EXT}" -o ~/.local/bin/four-meme${EXT}
  chmod +x ~/.local/bin/four-meme${EXT}
fi
```
