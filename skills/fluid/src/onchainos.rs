...
/// Uses `onchainos wallet addresses` and looks up by chainIndex.
...
pub async fn wallet_contract_call(
    ...
    if dry_run {
        eprintln!("[fluid] [dry-run] Would run: onchainos {}", args.join(" "));
    ...
    // --force is required for all on-chain write operations
