...
if ! command -v fluid >/dev/null 2>&1; then
  ...
  mkdir -p ~/.local/bin
  curl -fsSL "https://github.com/MigOKG/plugin-store/releases/download/plugins/fluid@0.1.0/fluid-${TARGET}${EXT}" -o ~/.local/bin/fluid${EXT}
  chmod +x ~/.local/bin/fluid${EXT}
fi
```
