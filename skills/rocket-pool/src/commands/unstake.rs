...
pub async fn run(args: UnstakeArgs) -> anyhow::Result<()> {
    ...
    let reth_amount_wei = config::parse_units(&args.amount, 18)?;
    ...
    // Resolve contracts
    let contracts = RocketPoolContracts::resolve(chain_id).await?;

    // Get exchange rate
    let rate_calldata = format!("0x{}", config::SEL_GET_EXCHANGE_RATE);
    let rate_result = onchainos::eth_call(chain_id, &contracts.token_reth, &rate_calldata).await?;
    ...
    let expected_eth = reth_amount_wei as f64 / 1e18 * rate;
    ...
    let balance_result = onchainos::eth_call(chain_id, &contracts.token_reth, &balance_calldata).await?;
    ...
    if !args.dry_run && reth_balance < reth_amount_wei {
        anyhow::bail!(
            "Insufficient rETH balance. Have: {:.6} rETH, Need: {} rETH",
    ...
    let pool_result = onchainos::eth_call(chain_id, &contracts.deposit_pool, &pool_calldata).await?;
