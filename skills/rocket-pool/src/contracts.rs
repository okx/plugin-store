...
impl RocketPoolContracts {
    ...
    pub async fn resolve(chain_id: u64) -> anyhow::Result<Self> {
        let deposit_pool = resolve_address(chain_id, config::KEY_DEPOSIT_POOL).await?;
        let token_reth = resolve_address(chain_id, config::KEY_TOKEN_RETH).await?;
        let network_balances = resolve_address(chain_id, config::KEY_NETWORK_BALANCES).await?;
        let node_manager = resolve_address(chain_id, config::KEY_NODE_MANAGER).await?;
        // For minipool manager, use known stable address as fallback
        let minipool_manager = resolve_address(chain_id, config::KEY_MINIPOOL_MANAGER).await
...
async fn resolve_address(chain_id: u64, key_hex: &str) -> anyhow::Result<String> {
    let calldata = format!("0x{}{}", config::SEL_GET_ADDRESS, key_hex);
    let result = onchainos::eth_call(chain_id, config::ROCKET_STORAGE, &calldata).await?;
