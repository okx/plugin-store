...
if ! command -v balancer-v2 >/dev/null 2>&1; then
  ...
  mkdir -p ~/.local/bin
  curl -fsSL "https://github.com/MigOKG/plugin-store/releases/download/plugins/balancer-v2@0.1.0/balancer-v2-${TARGET}${EXT}" -o ~/.local/bin/balancer-v2${EXT}
  chmod +x ~/.local/bin/balancer-v2${EXT}
fi
```
