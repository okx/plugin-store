...
pub async fn run(
    pool_id: &str,
    amounts: &[String], // amounts per token (use "0" to skip that token)
    chain_id: u64,
    dry_run: bool,
    confirm: bool,
) -> Result<()> {
    ...
    for (i, token) in tokens.iter().enumerate() {
        ...
        let raw = config::parse_units(&amounts[i], decimals)?;
        ...
        for (i, token) in tokens.iter().enumerate() {
            if raw_amounts[i] > 0 {
                ...
                if allowance < raw_amounts[i] {
                    ...
                    let approve_result =
                        onchainos::erc20_approve(chain_id, token, vault, u128::MAX, None, false, confirm).await?;
