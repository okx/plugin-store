...
async fn main() {
    ...
    let result = match cli.command {
        ...
        Commands::Swap { from, to, amount, pool, slippage, chain, dry_run, confirm } => {
            commands::swap::run(&from, &to, &amount, &pool, slippage, chain, dry_run, confirm).await
        }
        Commands::Join { pool, amounts, chain, dry_run, confirm } => {
            let amounts_vec: Vec<String> = amounts
                .split(',')
                .map(|s| s.trim().to_string())
                .collect();
            commands::join::run(&pool, &amounts_vec, chain, dry_run, confirm).await
        }
        Commands::Exit { pool, bpt_amount, chain, dry_run, confirm } => {
            commands::exit::run(&pool, &bpt_amount, chain, dry_run, confirm).await
        }
    };
