...
if ! command -v eigencloud >/dev/null 2>&1; then
  ...
  mkdir -p ~/.local/bin
  curl -fsSL "https://github.com/MigOKG/plugin-store/releases/download/plugins/eigencloud@0.1.0/eigencloud-${TARGET}${EXT}" -o ~/.local/bin/eigencloud${EXT}
  chmod +x ~/.local/bin/eigencloud${EXT}
fi
```
