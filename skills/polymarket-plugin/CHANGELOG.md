...
### v0.7.1 (2026-09-03) — 5-minute markets quote the live order book

- **fix**: `list-5m` and `get-series` now report the **live CLOB order-book mid** instead of Gamma's cached `outcomePrices`. That cache was measured 6m39s behind the book on a live 5-minute market — reporting `0.495` while the book was `0.82/0.83`, a 33-point error on a market that resolves every five minutes. Because the two commands hit different Gamma routes with independent caches, they also intermittently contradicted each other on the same `conditionId` (0.945 vs 0.385 observed, 9/9 samples over 28s). Output gains `up_bid`/`up_ask`/`down_bid`/`down_ask` (the tradeable sides), `price_source` (`clob_book` | `gamma_snapshot`), and `gamma_snapshot` for comparison. The snapshot is used only when the book cannot be read, and says so.
- **fix**: 5min and 15min crypto series are no longer modelled as NYSE-hours-only. They quote 24/7 — verified accepting orders at 04:41 ET with a live 99-level two-sided book — so `session` reported `"outside trading hours — next session opens in ~4h 59m"` on a slot whose own `accepting_orders` was `true`, while still handing the agent a ready-to-paste `buy` command. `session` is now derived from the market's `accepting_orders` and cannot contradict the slot it describes; `trading_hours` reads `24/7`.
- **fix**: `list-5m` paired Up/Down prices and token ids by **array position**, never reading Gamma's `outcomes` field. Gamma does not guarantee that order; if `Down` came first, `upTokenId` would be the Down token and `buy --token-id <upTokenId>` — a documented fast path that skips market lookup — would have silently bought the opposite side. Both are now matched by outcome name, as `get-series` already did.
- **docs**: SKILL.md — corrected the trading-hours section, added a price-source table telling the agent when a quote is tradeable versus a possibly-stale snapshot, and documented the new output fields on both commands.
- **chore**: `.claude-plugin/plugin.json` version was stuck at 0.6.2 while `plugin.yaml` read 0.7.0; both now read 0.7.1.
- **tests**: +7. `api.rs` unit tests (7) cover Up/Down mapping under a reversed `outcomes` array and best bid/ask extraction from unordered, empty, one-sided and unparseable books.
- **compat**: No CLI or flag changes. `up_price`/`down_price` keep their names and meaning (the price of that outcome) — they are now live rather than cached. Existing fields are unchanged; new fields are additive.

### v0.7.0 (2026-07-14) — Autotrade (copy-trading) authorization gate

- **feat**: `buy` / `sell` accept an optional `--autotrade-job <jobId>`. When present, the binary calls `onchainos agent autotrade-grant-check --job-id <id> --venue polymarket --action <buy|sell> --amount <amount|shares> --format json` (process contract frozen by the Dev PRD FR-3) **before any credential derivation, signing, or order placement**. The order proceeds only on exit 0 + stdout JSON `ok:true`; every other outcome — `ok:false` (reason passed through verbatim), non-zero exit, invalid JSON, timeout (10s, kill), missing binary, or an onchainos too old to know the subcommand — fail-closes with `{ok:false, error:{code:"autotrade_grant_denied", reason}}`. The plugin never reads the authorization grant itself; all grant/limit/subscription semantics live on the onchainos side, and error output never includes grant-file contents or paths.
- **feat**: jobId input validation — charset `[A-Za-z0-9_-]`, length 1..=128 (0x-prefixed hex jobIds pass naturally). Invalid values are rejected with `{ok:false, error:{code:"invalid_input", field:"autotrade-job"}}` without spawning any subprocess.
- **feat**: `--autotrade-job` + `--dry-run` skips the grant check (no real write occurs); the dry-run output carries `autotradeGrantCheck: "skipped (dry-run)"`. Successful authorized orders carry `autotradeJob: <jobId>` in the output for audit correlation.
- **docs**: SKILL.md — new "Autotrade (copy-trading) exception" section after the Live Trading Confirmation Protocol (three verifiable conditions + two hard prohibitions: never modify card parameters, never retry / re-route a rejected order); the "Do NOT use for" autonomous-execution entry now references that exception; `buy` / `sell` Flags tables document `--autotrade-job`; protocol rule 4 now points at the OnchainOS autotrade grant limits instead of a non-existent local skill config.
- **compat**: No breaking changes. Without `--autotrade-job`, behavior is byte-identical to 0.6.2 — the grant check is never invoked, outputs are unchanged, and the full 0.6.2 test suite passes without modification. Interactive usage keeps the unweakened confirmation protocol.
- **tests**: New `tests/autotrade_grant.rs` — 12 cases covering PRD AC-1..7 end-to-end by spawning the compiled binary against mock CLOB/RPC/geoblock servers and the `mock_onchainos.sh` fixture (new `agent autotrade-grant-check` mock with ok / deny / hang / bad-JSON / old-CLI modes). `get_clob_version` now resolves the CLOB base URL via `Urls::clob()` so `POLYMARKET_TEST_CLOB_URL` can inject a mock server (test-only hook; identical to the const in production).
