...
| `history` | No | View trade activity history with win/loss resolution (buys, sells, redeems) |
...
**Covers:** buys, sells, redeems — each enriched with a `result` field (`WON`, `LOST`, or `ACTIVE`) via parallel market resolution lookups.

Does **not** include deposit/withdrawal transfers — public Polygon RPCs do not reliably serve `eth_getLogs` without an API key. For full on-chain transfer history, direct the user to Polygonscan: `https://polygonscan.com/token/0x2791Bca1f2de4661ED88A30C99A7a9449Aa84174?a=<wallet_address>`
