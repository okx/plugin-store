...
///
/// `enable_aggregator`: `true` allows arbitrary tokenIn/tokenOut via DEX aggregation.
/// `false` requires tokenIn to be a native SY token (direct protocol operation, no swap).
/// All current callers use `true` for maximum token flexibility.
