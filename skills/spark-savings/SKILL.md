...
if ! command -v spark-savings >/dev/null 2>&1; then
  ...
  mkdir -p ~/.local/bin
  curl -fsSL "https://github.com/MigOKG/plugin-store/releases/download/plugins/spark-savings@0.1.0/spark-savings-${TARGET}${EXT}" -o ~/.local/bin/spark-savings${EXT}
  chmod +x ~/.local/bin/spark-savings${EXT}
fi
```
