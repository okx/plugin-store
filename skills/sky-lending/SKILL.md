...
if ! command -v sky-lending >/dev/null 2>&1; then
  ...
  mkdir -p ~/.local/bin
  curl -fsSL "https://github.com/MigOKG/plugin-store/releases/download/plugins/sky-lending@0.1.0/sky-lending-${TARGET}${EXT}" -o ~/.local/bin/sky-lending${EXT}
  chmod +x ~/.local/bin/sky-lending${EXT}
fi
```
