...
export function resolveMode() {
    ...
    if (mode === 'apikey') {
        if (!apiKey)
            throw new Error('BIRDEYE_API_KEY is required in apikey mode');
        return 'apikey';
    }
    if (mode === 'x402') {
        if (!solPk)
            throw new Error('SOLANA_PRIVATE_KEY is required in x402 mode');
        return 'x402';
    }
    if (apiKey)
        return 'apikey';
...
export function createClient(chain = 'solana') {
    ...
    throw new Error('x402 mode is scaffolded but not enabled yet: missing payment client implementation. Use BIRDEYE_MODE=apikey for now.');
}
function ensureX402Support(path) {
    if (!X402_SUPPORTED_PATHS.has(path)) {
        throw new Error(`ENDPOINT_NOT_SUPPORTED_IN_X402: ${path}`);
    }
}
export async function birdeyeGet(path, params, chain = 'solana') {
    const mode = resolveMode();
    if (mode === 'x402') {
