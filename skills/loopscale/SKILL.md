...
if ! command -v loopscale >/dev/null 2>&1; then
  ...
  mkdir -p ~/.local/bin
  curl -fsSL "https://github.com/MigOKG/plugin-store/releases/download/plugins/loopscale@0.1.0/loopscale-${TARGET}${EXT}" -o ~/.local/bin/loopscale${EXT}
  chmod +x ~/.local/bin/loopscale${EXT}
fi
```
