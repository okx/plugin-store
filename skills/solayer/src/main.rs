...
struct Cli {
    ...
    /// Broadcast the transaction on-chain (default: preview only)
    #[arg(long)]
    confirm: bool,
...
async fn main() {
    ...
    let result = match cli.command {
        ...
        Commands::Stake { amount } => {
            commands::stake::execute(amount, cli.confirm).await
        }
        Commands::Unstake { amount } => {
            commands::unstake::execute(amount, cli.confirm).await
        }
    };
