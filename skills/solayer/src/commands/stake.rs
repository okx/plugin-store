...
/// confirm: if false, show preview only; if true, broadcast on-chain
pub async fn execute(amount: f64, confirm: bool) -> anyhow::Result<Value> {
    ...
    if !confirm {
        return Ok(serde_json::json!({
            ...
            "data": {
                ...
                "amount_sol": amount,
                "ssol_mint": SSOL_MINT,
                "description": format!("Preview: would swap {} SOL → sSOL via onchainos swap execute (Jupiter routing). Re-run with --confirm to broadcast.", amount)
            }
        }));
    }
