...
pub struct SupplyArgs {
    /// Asset to supply (e.g. SUI, USDC, USDT, WETH)
    #[arg(long)]
    pub asset: String,

    /// Amount to supply (in token units, e.g. 1.5)
    #[arg(long)]
    pub amount: String,
    ...
    let preview = serde_json::json!({
        ...
        "move_call": {
            ...
            "args": [
                ...
                {"name": "coin_object", "description": "user coin object from wallet"},
                {"name": "amount", "value": args.amount}
            ]
        },
        "note": "Sui transaction submission requires a Sui wallet and native Sui CLI/SDK. Preview only — onchainos Sui support not yet available."
    });
