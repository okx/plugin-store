...
if ! command -v across >/dev/null 2>&1; then
  ...
  mkdir -p ~/.local/bin
  curl -fsSL "https://github.com/MigOKG/plugin-store/releases/download/plugins/across@0.1.0/across-${TARGET}${EXT}" -o ~/.local/bin/across${EXT}
  chmod +x ~/.local/bin/across${EXT}
fi
```
