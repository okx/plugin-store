...
if ! command -v gmx-v1 >/dev/null 2>&1; then
  ...
  mkdir -p ~/.local/bin
  curl -fsSL "https://github.com/MigOKG/plugin-store/releases/download/plugins/gmx-v1@0.1.0/gmx-v1-${TARGET}${EXT}" -o ~/.local/bin/gmx-v1${EXT}
  chmod +x ~/.local/bin/gmx-v1${EXT}
fi
```
