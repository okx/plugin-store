...
if ! command -v solv-solvbtc >/dev/null 2>&1; then
  ...
  mkdir -p ~/.local/bin
  curl -fsSL "https://github.com/MigOKG/plugin-store/releases/download/plugins/solv-solvbtc@0.1.0/solv-solvbtc-${TARGET}${EXT}" -o ~/.local/bin/solv-solvbtc${EXT}
  chmod +x ~/.local/bin/solv-solvbtc${EXT}
fi
```
