...
if ! command -v ion-protocol >/dev/null 2>&1; then
  ...
  mkdir -p ~/.local/bin
  curl -fsSL "https://github.com/MigOKG/plugin-store/releases/download/plugins/ion-protocol@0.1.0/ion-protocol-${TARGET}${EXT}" -o ~/.local/bin/ion-protocol${EXT}
  chmod +x ~/.local/bin/ion-protocol${EXT}
fi
```
