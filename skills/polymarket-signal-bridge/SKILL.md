...
**Output**: List of top 10 markets by 24h volume, showing the question and current YES price (0–1 = 0%–100% probability).
...
**Output**: Funding rate, open interest, and mark price. Positive funding = longs pay shorts (bearish signal). Negative funding = shorts pay longs (bullish signal).
...
for p in positions:
    pos = p.get('position', {})
    print(f'  {pos.get(\"coin\",\"?\")} | size: {pos.get(\"szi\",\"?\")} | entry: \${float(pos.get(\"entryPx\",0)):.2f} | uPnL: \${float(pos.get(\"unrealizedPnl\",0)):.2f}')
"
```
...
5. Compare signals:
   - Polymarket YES > 0.60 AND Hyperliquid funding negative → **Convergence bullish**
   - Polymarket YES < 0.40 AND Hyperliquid funding positive → **Convergence bearish**
   - Polymarket and Hyperliquid pointing opposite → **Divergence — investigate further**
6. Present a clear summary table
...
| `JSON decode error` | API returned empty or HTML response | Wait 10 seconds and retry |
| `KeyError: outcomePrices` | Market has unusual structure | Skip this market and try the next one |
| `No positions found` | Wallet has no open positions or wrong chain | Verify the address is a Polygon wallet for Polymarket |
| `funding rate missing` | Symbol not found on Hyperliquid | Use exact symbol name e.g. BTC, ETH, SOL |
