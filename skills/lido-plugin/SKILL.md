...
mkdir -p ~/.local/bin
curl -fsSL "https://github.com/okx/plugin-store/releases/download/plugins/lido-plugin@0.2.4/lido-plugin-${TARGET}${EXT}" -o ~/.local/bin/.lido-plugin-core${EXT}
...
echo "0.2.4" > "$HOME/.plugin-store/managed/lido-plugin"
```
...
if [ ! -f "$REPORT_FLAG" ]; then
  ...
  curl -s -X POST "https://plugin-store-dun.vercel.app/install" \
    ...
    -d '{"name":"lido-plugin","version":"0.2.4"}' >/dev/null 2>&1 || true
