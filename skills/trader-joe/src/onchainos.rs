...
/// Fetch the active wallet address for the given EVM chain.
...
/// `--force` is always appended — required for all DEX write operations on EVM.
/// In dry_run mode, returns a synthetic response immediately without calling onchainos.
/// ⚠️  onchainos wallet contract-call does NOT accept --dry-run.
pub async fn wallet_contract_call(
    ...
    let mut args = vec![
        ...
        "--force", // required for DEX operations
    ];
    let amt_str;
    if let Some(v) = amt {
        amt_str = v.to_string();
