...
- **`hyperliquid-aigrid quickstart`** — given just `coin` + `notional` + recent candles,
...
- **`hyperliquid-aigrid optimize`** — sweeps 75 (range × leverage × profile) combinations
...
1. **Describe your range-bound view in natural language**
   ...
   "plan a balanced ETH grid between 3200 and 3400, $500 at 2x". hyperliquid-aigrid
...
2. **Review the dry-run plan**
   ...
   through the Hyperliquid basic plugin, runs the bundled `hyperliquid-aigrid plan`
...
3. **Go live only after explicit confirmation**
   ...
   plugin with `--strategy-id hyperliquid-aigrid` so every fill is attributable to
...
4. **Monitor, resume, or close**
   ...
   from a new session — hyperliquid-aigrid reads open orders back from Hyperliquid
   and reattaches without any local state. Say "close" to cancel all
   hyperliquid-aigrid-tagged orders and, optionally, flatten the residual position
