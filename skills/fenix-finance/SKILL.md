...
if ! command -v fenix-finance >/dev/null 2>&1; then
  ...
  mkdir -p ~/.local/bin
  curl -fsSL "https://github.com/MigOKG/plugin-store/releases/download/plugins/fenix-finance@0.1.0/fenix-finance-${TARGET}${EXT}" -o ~/.local/bin/fenix-finance${EXT}
  chmod +x ~/.local/bin/fenix-finance${EXT}
fi
```
